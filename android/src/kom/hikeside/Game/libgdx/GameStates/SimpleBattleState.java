package kom.hikeside.Game.libgdx.GameStates;
...
import kom.hikeside.Game.libgdx.GameMechanics.SceneManager;
...
import kom.hikeside.Game.libgdx.GameObjects.Player;
import kom.hikeside.Models.LibraryObjects;
import kom.hikeside.Game.libgdx.Managers.GameStateManagement;
...
public class SimpleBattleState extends GameState {//обычная одиночная битва. соло, пвп, пве с разным количеством атакующих со всех сторон
    ...
    private void battleWon(){
        ...
        for(Enemy enemy : enemyArrayList)
            totalExperience += enemy.enemyModel.getExperience();
        totalExperience /= numberOfPlayers;

        bundle.setExperience(totalExperience);
        Log.d("battle", " is won");
        try {
    ...
    private void turn(){
        ...
        for(Enemy enemy : enemyArrayList)
            if(!enemy.isDead())
                sceneManager.makeAction(Randomizer.action(), enemy, playerArrayList.get(random.nextInt(playerArrayList.size())));//TODO сделать другую систему определения
    ...
    public void render() {
        ...
        for(StatusText status : sceneManager.statusArrayList){

            if(status.isShowing())
                status.render(batch);
            else{
                sceneManager.statusArrayList.remove(status);
            }
        ...
        buttonPlay.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                if(playerArrayList.get(0).turn) {//TODO определенный игрок ходит
                    ...
                    sceneManager.makeAction(text, playerArrayList.get(0), selectedEnemy);
